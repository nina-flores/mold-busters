...
for (seed in 1:100) {
  ...
  overall <- overall %>%
    ...
    summarize(visits_per_population = mean(visits_per_population),
              ...
              denominator_age = mean(denominator_age),
...
p_b <- ggplot(rubins_summary, aes(x = as.character(time), y = avg_estimate, color = MB)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci), width = 0.2) +
  facet_grid(.~MB, scales = "free_x", space = "free_x") +
  theme_minimal() +
  labs(
    title = "b. Null exposure aggregated",
    x = " ",
...
ggsave("negative-control-proximate-aggregated.pdf", p_b,width = 6, height = 4.5)
