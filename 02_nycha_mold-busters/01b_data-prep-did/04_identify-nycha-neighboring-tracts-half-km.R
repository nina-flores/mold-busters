...
nycha_buffer <- st_buffer(nycha_geom, dist = 500)

# Identify all tracts within .5km of any NYCHA tract
tracts_within_halfkm <- st_join(all_ct, nycha_buffer, join = st_intersects, left = FALSE)

a <- tracts_within_halfkm$GEOID.x %>%
  unique() %>%
  as.data.frame() %>%
  rename("geoid" = ".")

setwd("~/Desktop/projects/Mattlab/F31/data/processed data")
write.csv(a, "nycha-neighbors-500.csv", row.names = FALSE)  
