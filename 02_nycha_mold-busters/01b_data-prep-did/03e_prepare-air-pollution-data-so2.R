...
for (i in 8:15) {
  year <- 2008 + i
  folder <- paste0("w", i, "_so2300m")
  raster_path <- file.path(folder, "prj.adf")
  
  if (file.exists(raster_path)) {
    cat("Processing:", year, "\n")
    
    so2_raster <- raster(raster_path)
    crs(so2_raster) <- target_crs # Set CRS to match tracts
    
    # Extract weighted mean so2 for each tract
    so2_df <- raster::extract(
      x = so2_raster,
      y = tracts_NY,
      weights = TRUE,
      ...
      rename(annual_so2 = 2) %>% # second column is the extracted value
      mutate(GEOID = tracts_NY$GEOID,
             year = year)
    
    so2_list[[as.character(year)]] <- so2_df
  } else {
...
nyccas_all_years <- bind_rows(so2_list) 
...
write.csv(nyccas_all_years, "nyccas_so2_by_tract_2016_2023.csv", row.names = FALSE)
