...
glm_analytical_dat <- analytical_dat %>%
  ...
  mutate(
    ...
    n_repeat_reports = sum(n_repeat_reports, na.rm = TRUE),
    ...
    week_99th_ind_precip = sum(week_99th_ind_precip, na.rm = TRUE),
    week_99th_ind_precip = if_else(week_99th_ind_precip > 1, 1, week_99th_ind_precip),
  ...
  slice(1) %>% # be sure to remove duplicates
  ungroup() %>%
  ...
  mutate(
    ...
    sw_ind_0_99 = week_99th_ind_precip,
    sw_ind_1_99 = lag(week_99th_ind_precip, 1),
    sw_ind_2_99 = lag(week_99th_ind_precip, 2),
    sw_ind_3_99 = lag(week_99th_ind_precip, 3),
    sw_ind_4_99 = lag(week_99th_ind_precip, 4),
    sw_ind_5_99 = lag(week_99th_ind_precip, 5),
    sw_ind_6_99 = lag(week_99th_ind_precip, 6),
    sw_ind_7_99 = lag(week_99th_ind_precip, 7),
    sw_ind_8_99 = lag(week_99th_ind_precip, 8),
  ...
  mutate(any_founded = if_else(founded_mold >=1,1,0),
         ...
         any_repeat = if_else(n_repeat_reports >=1,1,0),
         any_non_severe = if_else(n_founded_no_severe>=1,1,0),
         multiple_report = if_else(mold_report >1,1,0),
         multiple_founded = if_else(founded_mold >1,1,0),
         multiple_severe = if_else(severe_mold >1,1,0),
         multiple_non_severe = if_else(n_founded_no_severe >1,1,0),
         year_month_MB_group = if_else(year_month_MB_factor == "Jul 2018" | year_month_MB_factor == "Apr 2019", "early", "late")) %>%
  mutate( may_2021 = if_else(year_week_date >= as.Date("2021-05-01"),1,0))
...
sum(glm_analytical_dat$founded_mold)
sum(glm_analytical_dat$founded_mold)
...
setwd("~/Desktop/projects/Mattlab/F31/Aim 1 - Severe weather and mold work orders/data/time_series")
saveRDS(glm_analytical_dat, file = "glm_analytical_dat.rds")
saveRDS(spatial_analytical_dat_frame, file = "spatial_analytical_dat_frame.rds")
